  ...
  drawFg = function(ctx, img, offset, yFunc, yoff, gap, maxH, segWidth) {
    ...
    var w = img.width;
    getImageFirstY(img);
    for (var i=0; i<1536*magF; i+=segW) {
      var x = off + i;
      var y = Math.floor(yFunc(offset+off+i, gap, true)-yoff);
      var sh = Math.max(0, h - Math.max(0, y+h-512*magF) - img.firstY);
      var sw = Math.max(0, Math.min(w-(i%w),segW));
      if (y > 512*magF || x > 1024*magF || x+sw <= 0 || sw <= 0 || sh <= 0) continue;
      ctx.drawImage(img,
        (i%w), img.firstY, sw, sh,
        x, y+img.firstY, sw, sh
      );
      if (y+ih < 512*magF) {
        for (var j=0; j<512*magF-y+ih; j+=24) {
          ctx.drawImage(img,
            (i%w), ih-24, sw, 24,
            x, y+ih+j, sw, 24
          );
  ...
  window.onload = function() {
    ...
    var bg_sky = load('bg_sky_field.png');
    var bg_forest = load('bg_forest_field.png');
    var bg_mountain = load('bg_mountains.png');
    ...
    startGame = function() {
      ...
      var drawLevel = function(st, offset, yoff, gap, lowres) {
        drawBg(ctx, bg_sky.getImage(lowres), st, 0.001, yoff, null, slow ? 400*magF : 300*magF);
        if (!slow) {
          drawBg(ctx, bg_mountain.getImage(lowres), offset, 0.01, yoff, null, 350*magF);
          drawBg(ctx, bg_forest.getImage(lowres), offset, 0.5, yoff, fn, 400*magF);
        }
        drawBg(ctx, bg_forest.getImage(lowres), offset, 0.75, yoff, fn, null);
        drawFg(ctx, fg.getImage(lowres), offset, fn, yoff, gap, null, slow ? 18 : 4);
      }
      ...
      draw = function() {
        ...
        if (!gameStarted) {
          ...
          if (gameOver) {
            ...
            if (titleA)
              ctx.drawImage(title.getImage(lowres), 320-title.getImage().width/2, titleY);
            ctx.globalAlpha = 1;
            drawBg(ctx, fg_front.getImage(lowres), offset, 1.15, yoff-30*magF, fn, null);
            if (e >= 2500) {
              ...
              ctx.fillText('Click to Play', 320-w/2, (420-10*(1-toff))*magF);
            }
          }
          if (frame <= 90) {
          ...
          if (y > 360) {
            ctx.fillStyle = 'rgba(255,255,255,'+((y-360) / 720)+')';
            ctx.fillRect(0,0,640,360);
          }
          if (y > 360+512) {
            playSfx(sfx.fall);
          ...
          ctx.fillText(rtxt, 640-rw-24*magF, 45*magF);
          if (y <= ground)
            drawShadow(ctx, shadow.getImage(lowres), offset, fn, yoff);
          ctx.drawImage(anim,
            w*f, h*hf, w, h,
            xoff*magF, y-yoff+80*magF, w, h
          );
          ...
          if (offset < 6000) {
            ...
            ctx.globalAlpha = 1-a;
            ctx.drawImage(inst, 320-inst.width/2, 360-inst.height-120*magF);
            ctx.globalAlpha = 1;
      ...
      musicToggle.onclick = function(ev) {
        musicOn = !musicOn;
        if (musicOn) {
          this.textContent = 'The Sound is On';
          music.play();
        } else {
          this.textContent = 'The Sound is Off';
        }
        ev.preventDefault();
        ev.stopPropagation();
      };
