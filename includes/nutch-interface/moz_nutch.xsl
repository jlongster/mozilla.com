...
<xsl:stylesheet version="1.0"
 ...
 >
	<xsl:output omit-xml-declaration="yes" method="xml" encoding="UTF-8" media-type="text/html" /> <!-- text/html is needed for the hack (see '.js' below) to work -->
	
	<!-- START SETTINGS -->
	<!-- text used; change this for translation and also some settings -->
	<xsl:variable name="use-doe-hack"></xsl:variable> <!-- leave this as true to enable a hack requiring the javascript file to handle web browsers that don't support disable-output-escaping (e.g. Mozilla-based ones); leave blank otherwise -->
			...
			<body>
				...
				<xsl:choose>
					<xsl:when test="$items">
						...
						<dl id="search-results"><xsl:apply-templates select="$items" /></dl>
						<!-- hack for xslt processors that do not respect disable-output-escaping="yes" --><xsl:if test="string-length($use-doe-hack)"><span id="encoding-test" style="display:none"><xsl:text disable-output-escaping="yes">&amp;amp;</xsl:text></span><script type="text/javascript" src="opensearch.js"></script></xsl:if>
						<!-- result navigation -->
						<p id="nav">
					...
					<xsl:otherwise><p id="no-results"><xsl:value-of select="$t-noresults" /></p></xsl:otherwise>
				</xsl:choose>
				...
				<div id="nutchfooter">
					...
					<p>This XSLT is &#169; <a href="http://a9.com/">A9.com, Inc</a>; see <a href="http://opensearch.a9.com/docs/stylesheet.jsp">full details</a>.</p>
				</div>
