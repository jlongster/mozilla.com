
<div id="main-content">

  <div class="sub-feature" id="intro">

    <h2>Elegí tu Persona.</h2>
    <p id="try">Pasá el mouse por encima de cada diseño para probarlo, hacé clic para aplicarlo</p>

    <a href="http://www.getpersonas.com/en-US/gallery/" id="personas-image-link">Personas</a>
    <iframe
        src="http://www.getpersonas.com/en-US/external/mozilla/firstrun.php"
        width="320"
        height="200">
    </iframe>

    <ul id="personas-link" class="link">
      <li>
          <a href="http://www.getpersonas.com/en-US/gallery/" id="see-all-personas">Mirá los más de <?=$personasnumber?></a>
      </li>
    </ul>
</div>

  <div class="sub-feature" id="sidebar">
    <h2>Seguí conectado</h2>
    <ul class="link">
      <li id="connect-twitter"><a href="http://twitter.com/firefox">Seguinos en Twitter</a></li>
      <li id="connect-facebook"><a href="http://www.facebook.com/Firefox">Hacete fan en Facebook</a></li>
      <li id="connect-blog"><a href="http://blog.mozilla.com/">Leé nuestro blog</a></li>
    </ul>
  </div>

  <div class="sub-feature<?=$oop_class;?>" id="personalize"><div>
    <h2>Otras maneras de personalizar</h2>
    <p>Adaptá Firefox a tu forma de navegar con miles de agregados gratuitos.</p>
    <ul class="link">
      <li>
          <a href="https://addons.mozilla.org/<?=$lang?>/firefox/">Explorar agregados</a>
      </li>
    </ul>
  </div></div>

<?=$oop;?>
</div>
<div id="footer">
    <div id="connect" class="sub-feature">
        <ul class="link social">
            <li id="connect-facebook"><a href="http://www.facebook.com/Firefox">Facebook</a></li>
            <li id="connect-twitter"><a href="http://twitter.com/firefox">Twitter</a></li>
        </ul>
    </div>
</div>
<div>

  <p id="sub-links">
      <a href="/<?=$lang?>/firefox/features/" id="features-link">Más características de Firefox 3.6</a>
      <a href="http://support.mozilla.com" id="support-link">Asistencia de Firefox</a>
  </p>

</div>
